...
export const AuthWrapper: React.FC<React.PropsWithChildren<{}>> = ({
	...
	const setUser = (user: UserModel): void => {
		console.log(user);
		localStorage.setItem(Shared.LocalStoregeKeys.USER, JSON.stringify(user));
		_setUser(user);
	};
	...
	useEffect(() => {
		const itemStr: UserModel =
			(JSON.parse(
				localStorage.getItem(Shared.LocalStoregeKeys.USER) as string
	...
	const signOut = (): void => {
		...
		localStorage.removeItem(Shared.LocalStoregeKeys.USER);
		history.push(`..${RoutePaths.Login}`);
	};
